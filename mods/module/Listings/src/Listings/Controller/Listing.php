...
class Listing extends AbstractAction
{
            ...
            if ($form->isValid() === true || isset($params[Form\Listing::BTN_PREV])) {
                ...
                if ($saveData === true) {
                    ...
                    if ($totalAmount > 0 && $userPaymentMode == 'live') {
                        ## -- START :: CHANGE -- [ MOD:- ADVANCED CLASSIFIEDS ]
                        $this->_helper->redirector()->redirect($paymentRedirectAction, 'payment', 'app',
                            array('id' => $listingId));
                        ## -- END :: CHANGE -- [ MOD:- ADVANCED CLASSIFIEDS ]
                    }
                    else {
    ...
    public function CalculatePostage()
    {
        ...
        $pickup = $this->getRequest()->getParam('pickup');
        ...
        if ($user instanceof UserModel) {
            ...
            if ($pickup == 1) {
                $shippingModel->setShippingDisplay(ShippingModel::DISPLAY_PICKUP);
            }
            else if ($pickup == -1) {
                $shippingModel->setShippingDisplay(ShippingModel::DISPLAY_POSTAGE);
            }
