...
class Checkout extends Cart
{
    ...
    public function setData(array $data = null, $flip = false)
    {
        ...
        if ($pickup) {
            $data['locationId'] = (isset($data[CartCheckout::PRF_PICKUP . 'country'])) ?
                $data[CartCheckout::PRF_PICKUP . 'country'] : null;
            $data['postCode'] = (isset($data[CartCheckout::PRF_PICKUP . 'zip_code'])) ?
                $data[CartCheckout::PRF_PICKUP . 'zip_code'] : null;
        }
        ...
        if ($user && !empty($data[CartCheckout::PRF_BLG . 'address_id'])) {
