...
use Ppb\Validate\SellersCredit as SellersCreditValidator,
...
class Purchase extends AbstractBaseForm
{
    ...
    public function __construct($formId, ListingModel $listing, UserModel $buyer = null, $action = null)
    {
        ...
        if ($listing->isProduct()) {
            $this->getView()->amount(false)->setConvert(true);
        }
        ## -- ./ADD -- [ MOD:- CURRENCY SELECTOR ]
        ...
        $this->getView()->amount(false)->setConvert(false);
    ...
    public function isValid()
    {
        ...
        if ($settings['enable_sellers_credit'] && $settings['mandatory_credit_payments']) {
            ...
            $amount = $listing->getProductPrice($request->getParam('product_attributes'));
            if ($this->hasElement('amount')) {
                $amount = $this->getData('amount');
            }

            $amount *= $quantity;

            $sellersCreditValidator = new SellersCreditValidator();
            $sellersCreditValidator->setBuyer(
                $this->getUser())
                ->setCurrency(
                    $listing->getData('currency'))
                ->setValue($amount);

            if (!$sellersCreditValidator->isValid()) {
                $valid = false;
                $this->setMessage(
                    $sellersCreditValidator->getMessage()
                );
            }
        }
