...
class Listings extends AbstractAction
{
    ...
    {        
        ...
        if ($this->getRequest()->getParam('option') == 'shipping-seller-buyer') {
            ...
            $this->_flashMessenger->setMessage(array(
                'msg'   => sprintf($translate->_("The shipping flag for invoice #%s has been updated."), $saleId),
                'class' => 'alert-success',
            ));
        }

        if ($this->getRequest()->getParam('option') == 'mark-as-paid') {
            $salesService = new SalesService();
            ...
            if ($sale->adminCanMarkAsPaidToSeller(true)) {
                $sale->save(array(
                    'flag_payment' => SaleModel::PAYMENT_PAID,
                ));
                $translate = $this->getTranslate();

                $this->_flashMessenger->setMessage(array(
                    'msg'   => sprintf($translate->_("The sale #%s has been marked as paid."), $saleId),
                    'class' => 'alert-success',
                ));
            }
            else {
                $this->_flashMessenger->setMessage(array(
                    'msg'   => $this->_("Error: the sale invoice cannot be found."),
                    'class' => 'alert-danger',
                ));
            }
