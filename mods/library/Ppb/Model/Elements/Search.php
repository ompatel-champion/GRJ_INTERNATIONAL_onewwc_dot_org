...
//            ->where('enable_auctions = ?', 1)
        ...
        if (array_key_exists('classified', $listingTypesMultiOptions)) {
            unset($listingTypesMultiOptions['classified']);
        }

        $listingType = (array)$this->getData('listing_type');

        if (in_array('classified', $listingType)) {
            $categoriesSelect->where('enable_classifieds = ?', 1);
        }
        else {
        ...
        $customFields = $this->getCustomFields()->getFields(
            array(
                ## -- CHANGE 1L -- [ MOD:- ADVANCED CLASSIFIEDS ]
                'type'         => (in_array('classified', $listingType)) ? 'classified' : 'item',
                'active'       => 1,
                'searchable'   => 1,
...
//        $listingTypesMultiOptions = $this->getListingTypes();
        ...
        $elements = array(
            ...
            array(
                ...
                'element'      => (!in_array('classified', $listingType)) ? 'checkbox' : false,
                'label'        => $this->_('Format'),
