...
class Shipping
{
    ...
    /**
     * USED BY POST CODE RADIUS ASSISTANT
     */
    const EQUATOR_LAT_MILE = 69.172;
    const RADIUS_DISTANCE = 10; // miles

    /**
     * RADIUS ASSISTANT - ONLY GET PICKUP LOCATIONS BASED ON
     * PROXIMITY IF THE COUNTRY ID IS UK
     */
    const UK_ID = 2083;

    /**
     * display which shipping options
     */
    const DISPLAY_ALL = 'all';
    const DISPLAY_POSTAGE = 'postage';
    const DISPLAY_PICKUP = 'pickup';
    ...
    /**
     *
     * array or string of address formatted to be usable by the geolocation service
     *
     * @var mixed
     */
    protected $_address;
    ...
    /**
     *
     * shipping methods display flag
     *
     * @var string
     */
    protected $_shippingDisplay = self::DISPLAY_ALL;
    ## -- END :: ADD -- [ MOD:- PICKUP LOCATIONS ]
    ...
    /**
     *
     * get shipping display flag
     *
     * @return string
     */
    public function getShippingDisplay()
    {
        return $this->_shippingDisplay;
    }

    /**
     *
     * set shipping display flag
     *
     * @param string $shippingDisplay
     *
     * @return $this
     */
    public function setShippingDisplay($shippingDisplay)
    {
        $this->_shippingDisplay = $shippingDisplay;
    ...
    public function calculatePostage()
    {
        ...
        if (!$this->_locationId) {
            ...
            throw new \RuntimeException($translate->_("No destination address has been set."));
        }
        ...
        if ($this->_isFreePostage()) {
            ...
            ## -- ONE LINE :: ADD -- [ MOD:- PICKUP LOCATIONS ]
            if ($this->getShippingDisplay() != self::DISPLAY_PICKUP) {
                switch ($this->getPostageType()) {
                    case self::POSTAGE_TYPE_ITEM:
                        ...
                        foreach ($this->_data as $storePickupLocation) {
                            $fldPostage = $storePickupLocation[self::DATA_LISTING][self::FLD_POSTAGE];
                            ...
                            if (!empty($fldPostage['price'])) {
                                foreach ($fldPostage['price'] as $key => $value) {
                                    ## -- ONE LINE :: CHANGE -- [ MOD:- PICKUP LOCATIONS ]
                                    $fldPostage['price'][$key] = doubleval($fldPostage['price'][$key]) * $storePickupLocation[self::DATA_QUANTITY];
                                }
                    ...
                    case self::POSTAGE_TYPE_FLAT:
                        ...
                        ## -- START :: CHANGE -- [ MOD:- PICKUP LOCATIONS ]
                        foreach ($this->_data as $storePickupLocation) {
                            $quantity += $storePickupLocation[self::DATA_QUANTITY];
                        }
                        ## -- END :: CHANGE -- [ MOD:- PICKUP LOCATIONS ]
        ...
        ## -- START :: CHANGE -- [ MOD:- PICKUP LOCATIONS ]
        if (($this->_canPickUp === true || $this->_mustPickUp === true) && $this->_shippingDisplay != self::DISPLAY_POSTAGE) {
            $storePickupLocations = $this->getStorePickupLocations();

            if (count($storePickupLocations) > 0) {
                $view = Front::getInstance()->getBootstrap()->getResource('view');

                /** @var \Ppb\Db\Table\Row\StorePickupLocation $storePickupLocation */
                foreach ($storePickupLocations as $storePickupLocation) {
                    $key = (-1) * $storePickupLocation->getData('id');
                    $distance = $storePickupLocation->getData('distance');
                    $collection = $storePickupLocation->getData('collection_days');

                    $result[$key] = array(
                        'currency' => $storePickupLocation->getData('currency'),
                        'price'    => $storePickupLocation->getData('price'),
                        'method'   => $translate->_(self::VALUE_PICK_UP)
                            . ': ' . $storePickupLocation->display(true),
                        'distance' => ($distance) ? ' - ' . round($distance, 1) . ' miles away' : null,
                        'carrier'  => '-',
                    );
                }
            }
...
//        ksort($result);
        ...
        if (empty($result)) {
            throw new \RuntimeException(!empty($carrierError) ? $carrierError : $translate->_("No shipping available for the selected destination.<br>
                Please adjust your address or enter an alternative destination."));
        }
        ## -- END :: CHANGE -- [ MOD:- PICKUP LOCATIONS ]
    ...
    public function calculateInsurance()
    {
        ...
        foreach ($this->_data as $row) {
            if (!empty($row[self::DATA_LISTING][self::FLD_INSURANCE])) {
                $insuranceAmount += doubleval($row[self::DATA_LISTING][self::FLD_INSURANCE]);
            }
        }
     ...
     */
    public function getStorePickupLocations()
    {
        $translate = $this->getTranslate();
        ...
        if ($this->_address) {
            ...
            $address = array(
                'zip_code' => $this->_postCode,
                'country'  => $this->_locationId,
            );
        }

        $geoLocationService = new Service\GeoLocation();
        $geoLocationService->setAddress($address);

        $coordinates = $geoLocationService->getCoordinates();

        // check if we have pickup in store locations
        $storePickupLocationsService = new Service\StorePickupLocations();

        $select = $storePickupLocationsService->getTable()->select()
            ->order('id DESC');

        if ($coordinates['latitude'] != 0 && $coordinates['longitude'] != 0) {
//KM            $select->columns('(6371 * acos(cos(radians(' . $coordinates['latitude'] . ')) * cos(radians(latitude)) * cos(radians(longitude) - radians(' . $coordinates['longitude'] . ')) + sin(radians(' . $coordinates['latitude'] . ')) * sin(radians(latitude)))) AS distance');
            $select->columns('(3959 * acos(cos(radians(' . $coordinates['latitude'] . ')) * cos(radians(latitude)) * cos(radians(longitude) - radians(' . $coordinates['longitude'] . ')) + sin(radians(' . $coordinates['latitude'] . ')) * sin(radians(latitude)))) AS distance');
            ...
            $select->where('address REGEXP \'"country";s:[[:digit:]]+:"' . $this->_locationId . '"\'');
        }

        $select->limit(10);

        /** @var \Ppb\Db\Table\Rowset\StorePickupLocations $storePickupLocations */
        $storePickupLocations = $storePickupLocationsService->fetchAll($select);
