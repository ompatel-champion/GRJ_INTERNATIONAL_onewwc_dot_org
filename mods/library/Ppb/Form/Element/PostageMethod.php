...
class PostageMethod extends Select
{
    ...
    public function setShippingModel($input, $locationId, $postCode)
    {
        ...
        foreach ($postage as $key => $row) {
            ...
            $attribute = $translate->_($row['method']) .
                ((!empty($row['distance'])) ? $row['distance'] : '') .
    ...
    /**
     *
     * set shipping model from array
     *
     * @param array $data
     *
     * @return $this
     */
    public function setShippingModelArray(array $data)
    {
        $this->setShippingModel($data['input'], $data['locationId'], $data['postCode']);
