...
class Sale extends AbstractRow
{
    ...
    const PAYMENT_IN_ESCROW = 5;
    ## -- END :: ADD -- [ MOD:- ESCROW PAYMENTS ]
    ## -- ONE LINE :: ADD -- [ MOD:- BANK TRANSFER ]
    const PAYMENT_PAID_BANK_TRANSFER = 4;
    ...
    ## -- START :: ADD -- [ MOD:- ESCROW PAYMENTS ]
    // MOD:- ESCROW_PAYMENTS
    const SHIPPING_SENT_TO_ADMIN = 3;
    const SHIPPING_RECEIVED = 4;
    ## -- END :: ADD -- [ MOD:- ESCROW PAYMENTS ]
    ...
    public static $paymentStatuses = array(
        ...
        ## -- START :: ADD -- [ MOD:- ESCROW PAYMENTS ]
        self::PAYMENT_IN_ESCROW           => 'Payment in Escrow',
        ## -- END :: ADD -- [ MOD:- ESCROW PAYMENTS ]
        ## -- ONE LINE :: ADD -- [ MOD:- BANK TRANSFER ]
        self::PAYMENT_PAID_BANK_TRANSFER  => 'Paid (Bank Transfer)',
    );
    ...
    /**
     *
     * shipping statuses array for escrow transactions
     *
     * @var array
     */
    public static $escrowShippingStatuses = array(
        self::SHIPPING_PROCESSING    => 'Processing',
        self::SHIPPING_SENT          => 'Posted/Sent to Buyer',
        self::SHIPPING_SENT_TO_ADMIN => 'Posted/Sent to Admin',
        self::SHIPPING_RECEIVED      => 'Received by Buyer',
        self::SHIPPING_PROBLEM       => 'Problem',
        self::SHIPPING_NA            => 'N/A',
    );
    ## -- END :: ADD -- [ MOD:- ESCROW PAYMENTS ]
    ...
    public function combineHash()
    {
        $array = array(
            ...
            ## -- ADD 1L -- [ MOD:- ESCROW PAYMENTS ]
            $this->getData('enable_escrow'),
        );
    ...
    public function canPayDirectPayment()
    {
        ...
        if ($this->isActive(false) &&
            (
                ($user['id'] == $this->getData('buyer_id') && !$this->isPaid()) ||
                ($user['role'] == 'Admin' && $this->getData('flag_payment') == self::PAYMENT_IN_ESCROW)
            )
        ) {
        ## -- END :: CHANGE -- [ MOD:- ESCROW PAYMENTS ]
            ...
            if ($this->canPaySellersCredit()) {
                $paymentGatewaysService = new Service\Table\PaymentGateways();
                $paymentGateway = $paymentGatewaysService->findBy('name', 'SellersCredit');

                $paymentMethods[] = array(
                    'id'   => $paymentGateway['id'],
                    'type' => 'direct',
                    'name' => $paymentGateway['name'],
                    'logo' => $paymentGateway['logo_path'],
                );
            }
    ...
    /**
     *
     * check if this sale can be paid for using the sellers credit method
     *
     * @param bool $apply if true, also charge the buyer and add the credit to the seller's account
     *
     * @return bool
     */
    public function canPaySellersCredit($apply = false)
    {
        $settings = $this->getSettings();

        if ($settings['enable_sellers_credit']) {
            /** @var \Ppb\Db\Table\Row\User $buyer */
            $buyer = $this->findParentRow('\Ppb\Db\Table\Users', 'Buyer');
            ...
            if ($buyer->canPaySellersCredit($amount, $currency) === true && $seller->acceptSellersCredit()) {
                if ($apply) {
                    $currenciesService = new Service\Table\Currencies();

                    $amountPaid = $currenciesService->convertAmount($amount, $currency);

                    $buyer->updateBalance($amountPaid);
                    $seller->updateBalance((-1) * $amountPaid);
                }
    ...
    /**
     *
     * seller can only mark as sent if it hasn't yet been marked as sent
     *
     * @return bool
     */
    public function canMarkShippingAsSent()
    {
        if (in_array($this->getData('flag_shipping'), array(self::SHIPPING_PROCESSING, self::SHIPPING_NA, self::SHIPPING_PROBLEM))) {
            return true;
        }
     ...
     * @return bool
     */
    public function canMarkShippingAsReceived()
    {
        if (in_array($this->getData('flag_shipping'), array(self::SHIPPING_SENT, self::SHIPPING_SENT_TO_ADMIN))) {
            return true;
        }
    ...
    /**
     *
     * admin can mark package as sent to buyer only if the flag is set as shipping to admin
     *
     * @param bool $adminFlag
     *
     * @return bool
     */
    public function adminCanMarkShippingAsSentToBuyer($adminFlag = false)
    {
        if ($adminFlag &&
            $this->getData('enable_escrow') &&
            $this->getData('flag_shipping') == self::SHIPPING_SENT_TO_ADMIN
        ) {
            return true;
        }
    ...
    /**
     *
     * admin can mark sale as paid offline only if the payment flag is set to "in escrow"
     *
     * @param bool $adminFlag
     *
     * @return bool
     */
    public function adminCanMarkAsPaidToSeller($adminFlag = false)
    {
        if ($adminFlag &&
            $this->getData('enable_escrow') &&
            $this->getData('flag_payment') == self::PAYMENT_IN_ESCROW
        ) {
            return true;
        }
