...
class SaleListing extends AbstractRow
{
     ...
     * DISCOUNT RULES MOD: apply discount rule as well
     *
    ...
    public function price($applyVoucher = false)
    {
        ...
        ## -- START :: ADD -- [ MOD:- DISCOUNT RULES @version 1.1 ]
        /** @var \Ppb\Db\Table\Row\Listing $listing */
        $listing = $this->findParentRow('\Ppb\Db\Table\Listings');
        if ($listing instanceof Listing) {
            if (($discountedPrice = $listing->discountedPrice($this->getData('product_attributes'))) > 0) {
                $price = $discountedPrice;
            }
        }
        ## -- END :: ADD -- [ MOD:- DISCOUNT RULES @version 1.1 ]
    ...
    /**
     *
     * generate link for activate / inactivate downloads buttons
     *
     * @param bool $admin
     *
     * @return array
     */
    public function updateDownloadLink($admin = false)
    {
        if ($admin) {
            return array(
                'module'          => 'admin',
                'controller'      => 'listings',
                'action'          => 'update-download-links',
                'sale_listing_id' => $this->getData('id')
            );
        }
        else {
            return array(
