...
class DirectPayment extends Service\Fees
{
    ...
    public function callback($ipn, array $post)
    {
        ...
        if ($ipn && !empty($post['escrow_buyer_admin'])) {
            $flag = SaleModel::PAYMENT_IN_ESCROW;
        }
        ## -- END :: ADD -- [ MOD:- ESCROW PAYMENTS ]
        ## -- START :: CHANGE -- [ MOD:- BANK TRANSFER ]
        if ($ipn == SaleModel::PAYMENT_PAID_BANK_TRANSFER) {
            $flag = $ipn;
        }
        ## -- END :: CHANGE -- [ MOD:- BANK TRANSFER ]        
        else if ($ipn) {
            $flag = SaleModel::PAYMENT_PAID_DIRECT_PAYMENT;
        ## -- START :: ADD -- [ MOD:- ESCROW PAYMENTS ]
        }
        else {
            $flag = 0;
