...
class Categories extends AbstractServiceTableRelational
{
    ...
    public function getColumns()
    {
        $columns = array(
            ...
            array(
                'label'      => $this->_('Auctions/ Products'),
                'class'      => 'size-tiny td-sm',
                'element_id' => 'enable_auctions',
                'parent_id'  => 0,
            ),
            array(
                'label'      => $this->_('Classifieds'),
                'class'      => 'size-tiny td-sm',
                'element_id' => 'enable_classifieds',
                'parent_id'  => 0,
            ),
            ## -- END :: ADD -- [ MOD:- ADVANCED CLASSIFIEDS ]
            array(
                'label'      => $this->_('Header Menu'),
                'class'      => 'size-mini td-sm',
    ...
    /**
     *
     * save different categories flags in all subcategories (supported - adult, enable_auctions, enable_categories)
     *
     * @param string $column
     * @param bool $enabled
     *
     * @return $this
     */
    protected function _saveCategoriesFlags($column, $enabled = true)
    {
        if ($enabled) {
            $ids = array();
    ...
    public function save($data)
    {
        ...
        if (!$parentId) {
            ## -- START :: CHANGE -- [ MOD:- ADVANCED CLASSIFIEDS ]
            $this->_saveCategoriesFlags('adult', $settings['enable_adult_categories']);
            $this->_saveCategoriesFlags('enable_auctions');
            $this->_saveCategoriesFlags('enable_classifieds');
            ## -- END :: CHANGE -- [ MOD:- ADVANCED CLASSIFIEDS ]
        }
