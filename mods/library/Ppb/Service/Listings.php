...
class Listings extends AbstractService
{
        ...
        public function select($selectType = self::SELECT_SIMPLE, array $params = null)
    {
            ...
            switch ($filter) {
                case 'active':
                    ...
                    if ($selectType != self::SELECT_MEMBERS) {
                        $select->where('l.hidden_product = ?', 0);
                    }
                    ## -- ./ADD -- [ MOD:- PRODUCT BUNDLES ]
                ...
                case 'discounted':
                    ...
                    $discountRulesSelect = $discountRulesService->getTable()->select()
                        ->where("start_date < NOW() OR start_date IS NULL")
                        ->where("expiration_date > NOW() OR expiration_date IS NULL")
                        ->where('active = ?', 1)
                        ->order(array('user_id ASC, priority DESC'));

                    $discountRulesTableAdapter = $discountRulesService->getTable()->getAdapter();

                    // group / filter by user id
                    $discountRules = $discountRulesService->fetchAll($discountRulesSelect);

                    $discountRulesQuery = array(
                        'local'  => array(),
                        'global' => array(),
                    );

                    $discountRulesQuery['local'] = array(
                        $discountRulesTableAdapter->quoteInto('l.enable_discount_rule = ?', 1),
                        '(l.discount_start_date < NOW() OR l.discount_start_date IS NULL)',
                        '(l.discount_expiration_date > NOW() OR l.discount_expiration_date IS NULL)',
                    );

                    $stopFurtherRules = array();

                    foreach ($discountRules as $discountRule) {
                        $discountRuleUserId = intval($discountRule['user_id']);

                        if (!array_key_exists($discountRuleUserId, $stopFurtherRules)) {
                            $stopFurtherRules[$discountRuleUserId] = 0;
                        }

                        if (!$stopFurtherRules[$discountRuleUserId]) {
                            $discountRuleQuery = array();

                            if ($discountRuleUserId) {
                                $discountRuleQuery[] = $discountRulesTableAdapter->quoteInto('l.user_id = ?', $discountRule['user_id']);
                            }

                            $discountRuleConditions = $discountRule['conditions'];

                            if (!empty($discountRuleConditions)) {
                                $discountRuleQuery[] = "("
                                    . str_replace(
                                        array('listingId', 'userId', 'purchasedListingId', 'price', 'name', 'description'),
                                        array("l.id", "'" . intval($loggedInUserId) . "'", "s.listing_id", "l.buyout_price", "l.name", "l.description"),
                                        $discountRuleConditions
                                    )
                                    . ")";
                            }

                            $discountRulesQuery['global'][] = '(' . implode(' AND ', $discountRuleQuery) . ')';
                        }

                        $stopFurtherRules[$discountRuleUserId] = $discountRule['stop_further_rules'];
                    }

                    $discountRulesQueryString = '(' . implode(' AND ', $discountRulesQuery['local']) . ')';

                    if (count($discountRulesQuery['global']) > 0) {
                        $discountRulesQueryString .= ' OR ' . '(' . implode(' OR ', $discountRulesQuery['global']) . ')';
                    }

                    $discountRulesQueryString = str_ireplace(
                        array('&amp;', '&#039;', '&quot;', '&lt;', '&gt;', '&nbsp;'), array('&', "'", '"', '<', '>', ' '), $discountRulesQueryString);
                    $select->where($discountRulesQueryString);
                    break;
                ## -- ./ADD -- MOD:- DISCOUNTS
            }
        }
        ...
        $customFieldsTypes = array('item');
        if (in_array('all', (array)$listingType)) {
            $customFieldsTypes[] = 'classified';
        }
        else if (in_array('classified', (array)$listingType)) {
            if (count($listingType) > 1) {
                $customFieldsTypes[] = 'classified';
            }
            else {
                $customFieldsTypes = array('classified');
            }
        }
    ...
    protected function _prepareSaveData($data = array())
    {
        ...
        if ($data['discount_start_date'] == '') {
            $data['discount_start_date'] = new Expr('null');
        }
        if ($data['discount_expiration_date'] == '') {
            $data['discount_expiration_date'] = new Expr('null');
        }
