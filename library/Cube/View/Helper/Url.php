...
class Url extends AbstractHelper
{
    ...
    public function url($params, $name = null, $addGetParams = false, array $skipParams = null, $addBaseUrl = true, $cleanString = true)
    {
        ...
        return $router->assemble($params, $name, $addBaseUrl, $addGetParams, $skipParams);
    }

    /**
     *
     * formats a string for url display purposes
     *
     * @param string $string
     * @param array  $replace
     * @param string $delimiter
     *
     * @return string|null           the formatted string or null if the input is not a string
     */
    public static function cleanString($string, $replace = array(), $delimiter = '-')
    {
        if (!is_string($string)) {
            return $string;
        }
