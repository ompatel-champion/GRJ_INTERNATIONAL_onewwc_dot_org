...
class Offer extends AbstractRow
{
    ...
    /**
     *
     * get sale model if available
     *
     * @return \Ppb\Db\Table\Row\Sale|null
     */
    public function getSale()
    {
        ...
        $saleListing = $this->findParentRow('\Ppb\Db\Table\SalesListings');
     ...
     *
     * @8.2 save sale_listing_id to the offers table
     *
     * @return bool
     */
    public function accept()
    {
        ...
        if ($this->canAccept($listing)) {
            ...
            /** @var \Ppb\Db\Table\Row\Sale $sale */
            $sale = $service->findBy('id', $service->getSaleId());

            if ($sale !== null) {
                $saleListing = $sale->findDependentRowset('\Ppb\Db\Table\SalesListings')->getRow(0);

                if ($saleListing !== null) {
                    $this->save(array(
                        'sale_listing_id' => $saleListing->getData('id'),
                    ));
                }
            }
