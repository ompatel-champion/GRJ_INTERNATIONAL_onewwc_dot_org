...
class Countdown extends AbstractHelper
{
    ...
    public function display()
    {
        ...
        $timeLeft = $this->_date - time();

        if ($timeLeft <= 0) {
            return '<span class="closed">' . $translate->_('Closed') . '</span>';
        }
    ...
    protected function _generateJavascript()
    {
        ...
        $scriptHelper->addBodyCode('<script type="text/javascript" src="' . $this->_baseUrl . '/js/jquery.countdown.min.js"></script>')
            ->addBodyCode("<script type=\"text/javascript\">" . "\n"
                ...
                . $this->countdownJavascript()                
                . " }); " . "\n"
                . "</script>");
    }

    public function countdownJavascript()
    {
        $translate = $this->getTranslate();

        return "$('[data-countdown]').each(function () {
            ...
            element.countdown(finalDate, function (event) {
                ...
                element.html('<span class=\"text-info\">" . $translate->_('Please wait..') . "</span>');
            });
        });";
    }
