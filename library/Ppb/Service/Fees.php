...
class Fees extends AbstractService
{
     ...
     */
    const SETUP = 'setup';
    const CLASSIFIED_SETUP = 'classified_setup';
    const SALE = 'sale';
    const HPFEAT = 'hpfeat';
    ...
    public function getTaxType()
    {
        if (!$this->_taxType instanceof TaxType) {
            ...
            if ($settings['enable_tax_fees'] && $settings['tax_fees_type']) {
                ...
                $taxTypesIds = array_filter((array)\Ppb\Utility::unserialize($settings['tax_fees_type']));
                ...
                if (count($taxTypesIds) > 0) {
                    ...
                    foreach ($taxTypes as $taxType) {
                        ...
                        if (
                            isset($address['country']) && isset($address['state']) && is_array($locationsIds) &&
                            (in_array($address['country'], $locationsIds) || in_array($address['state'], $locationsIds))
                        ) {
...
//            return $this->_applyVoucher($amount, $settings['currency']);
    ...
    public function save($data)
    {
        ...
        if (isset($data['calculation_type'])) {
            if (!is_array($data['calculation_type'])) {
        ...
        if ($this->hasTiers()) {
            if (isset($data['tier_from'])) {
                foreach ($data['tier_from'] as $key => $tierFrom) {
                    if (!empty($data['amount'][$key])) {
                        $row = $where = null;

                        if (!empty($data['id'][$key])) {
                            $where = $table->getAdapter()->quoteInto('id = ?', intval($data['id'][$key]));
                        ...
                        $array = array(
                            ...
                            'calculation_type' => $data['calculation_type'][$key],
                            'category_id'      => (!empty($categoryId)) ? $categoryId : new Expr('null'),
                            'tier_from'        => $data['tier_from'][$key],
                            'tier_to'          => $data['tier_to'][$key],
                        );
