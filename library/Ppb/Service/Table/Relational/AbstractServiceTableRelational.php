...
abstract class AbstractServiceTableRelational extends AbstractServiceTable
{
    ...
    /**
     *
     * relational table navigation object
     *
     * @var \Cube\Navigation
     */
    protected $_data;
        ...
        foreach ($rows as $row) {
            ...
            if ($fullName === true) {
                if (isset($row['full_name'])) {
                    ...
                    $parts = explode(self::NAME_SEPARATOR, $this->getFullName($row['id']));
                }
     ...
     * @8.0 - option to not translate breadcrumbs automatically, needed for creating category slugs / full names on multi language sites
     * @8.0 - option to return breadcrumbs as an array of objects rather than strings
     *
     * @param int  $id
     ...
     * @return array
     */
    public function getBreadcrumbs($id, $translate = true, $object = false)
    {
        $breadcrumbs = array();

        if ($id) {
            $translateAdapter = $this->getTranslate();
            ...
            do {
                ...
                $id = 0;
                if ($row !== null) {
                    $breadcrumbs[$row['id']] = ($object) ? $row : (($translate) ? $translateAdapter->_($row['name']) : $row['name']);
                    $id = $row['parent_id'];
                }
            } while ($id > 0);
        }
     ...
     * @return string|null
     */
    public function getFullName($id, $separator = null, $translate = true)
    {
        if ($separator === null) {
            $separator = self::NAME_SEPARATOR;
        }

        $name = implode($separator, array_values($this->getBreadcrumbs($id, $translate)));
