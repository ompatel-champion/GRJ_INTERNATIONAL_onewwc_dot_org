...
class UserNotification extends BaseMail
{
    ...
    public function offerDeclined(ListingModel $listing, OfferModel $offer)
    {
        ...
        $type = ($user['id'] == $listing['user_id']) ? 'selling' : 'buying';

        $this->setData(array(
            'listing' => $listing,
            'offer'   => $offer,
            'link'    => array('module' => 'members', 'controller' => 'offers', 'action' => 'details', 'type' => $type, 'id' => $offer['topic_id']),
        ));
    ...
    public function offerWithdrawn(ListingModel $listing, OfferModel $offer)
    {
        ...
        $type = ($receiver['id'] == $listing['user_id']) ? 'selling' : 'buying';
