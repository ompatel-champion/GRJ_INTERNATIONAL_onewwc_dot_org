...
class PostageSetup extends Form\AbstractBaseForm
{
    ...
    public function __construct($action = null)
    {
        ...
        $freePostageAmount = $this->createElement('\\Ppb\\Form\\Element\\LocalizedNumeric', ShippingModel::SETUP_FREE_POSTAGE_AMOUNT);
        ...
        $shippingCarriersRowset = $shippingCarriersService->fetchAll('enabled = 1')->toArray();
        ...
        if ($user instanceof UserModel) {
            if ($user->isPostmenShippingApi()) {
                ...
                $postmenShipperAccounts = $postmenShipperAccountsService->fetchAll(
                    ...
                    ->where('user_id = ?', $user->getData('id'))
                );

                /** @var \Ppb\Db\Table\Row\PostmenShipperAccount $postmenShipperAccount */
                foreach ($postmenShipperAccounts as $postmenShipperAccount) {
                    if ($postmenShipperAccount->isEnabled()) {
                        $shippingCarriersRowset[] = array(
                            'name' => $postmenShipperAccount->getSlug(),
                            'desc' => $postmenShipperAccount->getDescription(),
                        );
                    }
                }
            }
        ...
        $postageFlatFirst = $this->createElement('\\Ppb\\Form\\Element\\LocalizedNumeric', ShippingModel::SETUP_POSTAGE_FLAT_FIRST);
        ...
        $postageFlatAddl = $this->createElement('\\Ppb\\Form\\Element\\LocalizedNumeric', ShippingModel::SETUP_POSTAGE_FLAT_ADDL);
        ...
        $dimensionsUom = $this->createElement('radio', ShippingModel::SETUP_DIMENSIONS_UOM);
        $dimensionsUom->setLabel(ShippingModel::$postageSetupFields[ShippingModel::SETUP_DIMENSIONS_UOM])
            ->setDescription('Select the dimensions unit of measurement that you will be using for your listings.')
            ->setMultiOptions(ShippingModel::$dimensionsUom)
            ->setValue(key(ShippingModel::$dimensionsUom))
            ->setRequired();
        $this->addElement($dimensionsUom);
    ...
    public function setData(array $data = null)
    {
        ...
        if ($data[ShippingModel::SETUP_SHIPPING_LOCATIONS] != ShippingModel::POSTAGE_LOCATION_CUSTOM) {
            $this->getElement(ShippingModel::SETUP_LOCATION_GROUPS)
                ->clearValidators();
        }
